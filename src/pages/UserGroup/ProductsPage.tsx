...
export const ProductsPage = () => {
    ...
    const [data, isLoading, error] = useApi<
        "productsList",
        ProductModelDataResult
    >("productsList", { BrandGuid: id }, {}, true);

    //фильтры
    const [filtersData, ,] = useApi<
        "productCategoriesList",
        ProductCategoryModelDataResult
    >("productCategoriesList", { PageSize: 248 }, {}, true);
            ...
            prevCategories.map((category) => {
                const updateCategory = (cat: Category): Category => {
                    if (cat.title.name === categoryName) {
                        return {
                            ...cat,
                            leafCategoryStates: cat.leafCategoryStates.map(
                                (state) => {
                                    if (filterName in state) {
                                        return {
                                            [filterName]: {
                                                ...state[filterName],
                                                status: !state[filterName]
                                                    .status,
                                            },
                                        };
                                    }
    ...
    return (
        <div>
            ...
            <Modal
                ...
                additionalStyles={{
                    container:
                        "fixed bg-white-fon inset-0 overflow-y-scroll my-16 flex items-center justify-center",
            ...
            >
                ...
                <div className="mt-2 w-1/2 mx-auto">
                    <Button
                        ...
                        onClick={() => {
                            setModal(false);
                            updateСheckedCategories(filterCategories);
                        }}
