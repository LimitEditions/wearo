...
export const Promo = () => {
    ...
    const [getCodes, codesLoading] = useApi<
        "promotionsCodesList",
        StringDataResult
    >(
        "promotionsCodesList",
    ...
    // получить новый код
    const [shouldExecuteNewCode, setShouldExecuteNewCode] =
        useState<boolean>(false);
    const [getNewCode, getNewCodeIsLoading, getNewCodeError] = useApi<
        "promotionsCodesUpdate",
        string
    >("promotionsCodesUpdate", { id: id }, config, shouldExecuteNewCode);
        ...
        <div className="relative h-[calc(90vh-80px)] px-2">
            ...
            {data && (
                <div>
                    <Photo
                        id={data.imageGuid || ""}
                        styles={"w-16 object-cover my-3"}
                        alt={"фото"}
                    />
                    <div className="w-full bg-yellow h-36 my-6 p-3 space-y-2">
                        <h1 className="uppecase text-lg font-semibold">
                            {data.name}
                        </h1>
                        ...
                        <ul className="list-decimal space-y-2 px-7 py-2">
                            {data.products?.map((prod) => {
                                return (
                                    <li key={prod.guid}>
                                        <Link
                                            to={`/product/${prod.productGuid}`}
                                        >
                                            Изделие (жми для перехода)
                                        </Link>
                                    </li>
                                );
                            })}
                        ...
                        {codes.map((code, index) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <Button
                                        showButton={true}
                                        styles="ml-3 text-xs cursor-pointer"
                                        onClick={copyCode}
                                    >
